...
def pars():
    ...
    for i in tqdm(timer):
        ...
        df.to_csv(r''+chrome+'\\'+zap_Input.get()+'.csv', encoding="utf-8", index=False)
...
def sbor_api():
    ...
    try:
        ...
        if req=='':
            reque = 'https://opendata.mkrf.ru/v2/museum-exhibits/$?f={"data.museum.code":{"$'+how_req+'":"'+mus+'"}'
            reque = re.sub(' ', '%20', reque)
            reque = re.sub('"', '%22', reque)
            reque = re.sub('{', '%7B', reque) 
            reque = re.sub('}', '%7D', reque)
        else:
            reque = 'https://opendata.mkrf.ru/v2/museum-exhibits/$?f={"data.name":{"$'+how_req+'":"'+req+'"},"data.museum.code":{"$eq":"'+mus+'"}'
            reque = re.sub(' ', '%20', reque)
            reque = re.sub('"', '%22', reque)
            reque = re.sub('{', '%7B', reque) 
            reque = re.sub('}', '%7D', reque)
        ...
        df = pd.DataFrame(data)
        df.to_csv(r''+chrome+'\\'+zap_Input3.get()+'.csv', encoding="utf-8", index=False)
...
def proc_data_choice():
    ...
    if combobox4.get() == 'География': proc_data_1()
    elif combobox4.get() == 'Даты': proc_data_2()
    elif combobox4.get() == 'Леммы': proc_data_3()
    elif combobox4.get() == 'Коллокации': proc_data_4()
...
def proc_data_1():
    ...
    try:
        ...
        ### Проверка ###
        df = pd.read_csv(fileName2, encoding="utf-8")
        df[""+combobox.get()+"(обр.)"].tolist()
        ...
        df.insert(6, ""+combobox.get()+"(обр.)", vv)

        df.to_csv(fileName2, encoding="utf-8", index=False)

        languages = list(df.columns.values)
        languages_var.set(languages[0])
        combobox.configure(values=languages,textvariable=languages_var)
...
def proc_data_2():
    ...
    try:
        ...
        df.insert(5, ""+combobox.get()+"(обр.)", vv)
        df.to_csv(('\\'.join(fileName2.split('\\')[:-1])+'\\'.join(fileName2.split('\\')[-1:])), encoding="utf-8", index=False)
...
def proc_data_3():
    ...
    try:
        ...
        ### Проверка ###
        file_ = open('\\'.join(fileName2.split('\\')[:-1])+combobox.get()+".txt", "r", encoding="utf-8")
        ...
        with open('\\'.join(fileName2.split('\\')[:-1])+combobox.get()+'.txt', 'w', encoding="utf-8") as filehandle: 
...
def delete_new_col():
    ...
    try:
        df = pd.read_csv(fileName2, encoding="utf-8")
        df = df.drop(columns=[combobox.get()])
        df.to_csv(fileName2, encoding="utf-8", index=False)
        ...
        mb.showwarning("Внимание!", "Нет обработанного столбца с таким именем")
...
def analysis_choice():
    ...
    #['Гистограмма по количеству','Облако лемм','Облако коллокаций','Пропуски в данных','Распределение дат']

    global combobox3

    if combobox3.get() == 'Гистограмма': analysis1()
    elif combobox3.get() == 'Облако лемм': analysis2()
    elif combobox3.get() == 'Пропуски в данных': analysis3()
    elif combobox3.get() == 'Распределение дат': analysis4()
...
def analysis1():
    ...
    fig.savefig('\\'.join(fileName.split('\\')[:-1])+head+".png", bbox_inches="tight")
...
def analysis2():
    ...
    if vvod == 'Название': vvod = 'Название'
    elif vvod == 'Описание': vvod = 'Описание'

    with open('\\'.join(fileName.split('\\')[:-1])+vvod+'.txt', 'r', encoding="utf-8") as filehandle: 
    ...
    stop_words = []
    try:
        with open('\\'.join(fileName.split('\\')[:-1])+"stopwords.txt", encoding="utf-8") as file: stop_words = [row.strip() for row in file]
    except FileNotFoundError:
        stop_words = []
    ...
    plt.savefig('\\'.join(fileName.split('\\')[:-1])+head+".png", bbox_inches='tight')
...
def analysis4():
    ...
    plt.title(str(head)+' '+str(round(kol/len(df)*100,2))+'% ('+str(kol)+' / '+str(len(df))+')')
    ...
    fig.savefig('\\'.join(fileName.split('\\')[:-1])+head+".png", dpi=100)
...
title_zapch.grid(row=1, column=2, ipadx=0, ipady=0, padx=15, pady=0, columnspan=2) 
zapch_Input = Entry(frame_for_center, bg='white', font=("Century Gothic", 13, "bold"), justify=CENTER, width=25)
zapch_Input.grid(row=2, column=2, ipadx=0, ipady=0, padx=15, pady=0, columnspan=2) 

title_muz = Label(frame_for_center2, text='ID музея', bg='gray',fg='white', font=("Century Gothic", 10))
title_muz.grid(row=3, column=0, ipadx=0, ipady=0, padx=15, pady=0, columnspan=2) 
muz_Input = Entry(frame_for_center2, bg='white', font=("Century Gothic", 13, "bold"), justify=CENTER, width=25)
muz_Input.grid(row=4, column=0, ipadx=0, ipady=0, padx=15, pady=0, columnspan=2) 

title_pyt = Label(frame_for_center2, text='Путь к папке', bg='gray',fg='white', font=("Century Gothic", 10))
title_pyt.grid(row=3, column=2, ipadx=0, ipady=0, padx=15, pady=0, columnspan=2) 
pyt_Input = Entry(frame_for_center2, bg='white', font=("Century Gothic", 13, "bold"), justify=CENTER, width=25)
pyt_Input.grid(row=4, column=2, ipadx=0, ipady=0, padx=15, pady=0, columnspan=2) 
...
f1 = frame_for_hight.place(in_=frame1, anchor="c", relx=.50, rely=.15)

frame_for_center = Frame(background="gray")
f2 = frame_for_center.place(in_=frame1, anchor="c", relx=.50, rely=.35)

frame_for_center2 = Frame(background="gray")
f3 = frame_for_center2.place(in_=frame1, anchor="c", relx=.50, rely=.51)

frame_for_center3 = Frame(background="gray")
f4 = frame_for_center3.place(in_=frame1, anchor="c", relx=.50, rely=.73)
...
title_api3.grid(row=1, column=2, ipadx=0, ipady=0, padx=15, pady=0, columnspan=2)
api_Input3 = Entry(frame_for_center, bg='white', font=("Century Gothic", 13, "bold"), justify=CENTER, width=25)
api_Input3.grid(row=2, column=2, ipadx=0, ipady=0, padx=15, pady=0, columnspan=2)

try: 
    api_key_file = open(r"API-KEY.txt", "r", encoding="utf-8").read().split('\n')[0]
    api_Input3.insert(0, api_key_file)
except:
    pass


title_muz3 = Label(frame_for_center2, text='КОПУК музея', bg='gray',fg='white', font=("Century Gothic", 10))
title_muz3.grid(row=3, column=0, ipadx=0, ipady=0, padx=15, pady=0, columnspan=2) 
muz_Input3 = Entry(frame_for_center2, bg='white', font=("Century Gothic", 13, "bold"), justify=CENTER, width=25)
muz_Input3.grid(row=4, column=0, ipadx=0, ipady=0, padx=15, pady=0, columnspan=2)

title_pyt3 = Label(frame_for_center2, text='Путь к папке', bg='gray',fg='white', font=("Century Gothic", 10))
title_pyt3.grid(row=3, column=2, ipadx=0, ipady=0, padx=15, pady=0, columnspan=2) 
pyt_Input3 = Entry(frame_for_center2, bg='white', font=("Century Gothic", 13, "bold"), justify=CENTER, width=25)
pyt_Input3.grid(row=4, column=2, ipadx=0, ipady=0, padx=15, pady=0, columnspan=2)
...
frame_0 = Frame(background="gray")
f0 = frame_0.place(in_=frame3, anchor="c", relx=.50, rely=.20)

frame_for_hight = LabelFrame(frame_0, background="gray", text='Способ визуализации', bg='gray', fg='white', font=("Century Gothic", 10), labelanchor="n")
frame_for_hight.grid(row=0, column=0, ipadx=0, ipady=0, padx=8, pady=0) 

frame_for_center = LabelFrame(frame_0, background="gray", text='Столбец для визуализации', bg='gray', fg='white', font=("Century Gothic", 10), labelanchor="n")
frame_for_center.grid(row=0, column=1, ipadx=0, ipady=0, padx=8, pady=0) 
...
f4 = frame_for_center4.place(in_=frame3, anchor="c", relx=.50, rely=.78)
...
languages3 = ['Гистограмма','Облако лемм','Облако коллокаций','Пропуски в данных','Распределение дат']
languages_var3 = StringVar()
combobox3 = ttk.Combobox(frame_for_hight, textvariable=languages_var3, height=10, width=17, values=languages3, font=("Century Gothic", 12, "bold"))
languages_var3.set(languages3[0])
combobox3.grid(row=1, column=0, ipadx=5, ipady=0, padx=7, pady=7)
...
languages2 = []
languages_var2 = StringVar()
combobox2 = ttk.Combobox(frame_for_center, textvariable=languages_var2, height=10, width=17, values=languages2, font=("Century Gothic", 12, "bold"))
combobox2.grid(row=1, column=0, ipadx=5, ipady=0, padx=7, pady=5)

def rep2():
    global fileName
    df = pd.read_csv(fileName, encoding="utf-8")
    languages2 = list(df.columns.values)
    languages_var2.set(languages2[0])
    combobox2.configure(values=languages2,textvariable=languages_var2)

icon_plus_rep2= PhotoImage(file="rep.png")
photoimage_rep2 = icon_plus_rep2.subsample(2, 2)
btnm_ob2 = Button(frame_for_center, image=photoimage_rep2, command=rep2, text='-', bg='white', height=23, width=23, font=("Century Gothic", 10, "bold"))
btnm_ob2.grid(row=1, column=1, ipadx=0, ipady=0, padx=7, pady=5)
...
def file_nm():
    ...
    title_file_name.configure(text=fileName[:30]+'...'+fileName[-30:])
    rep2()

file_in = Button(frame_for_center4, command=file_nm, text='Выбрать файл', bg='white', height=1, width=24, font=("Century Gothic", 10, "bold"))
file_in.grid(row=0, column=0, ipadx=0, ipady=0, padx=15, pady=0)

def color_():
    global clr
    result = colorchooser.askcolor(initialcolor="#1f77b4")
    clr = result[1]

icon_color = PhotoImage(file="color.png")
photoimage_color = icon_color.subsample(2, 2)
btninfo = Button(frame_for_center4, command=color_,image=photoimage_color, text='i', bg='white', height=25, width=25, font=("Century Gothic", 10, "bold"), justify=CENTER)
btninfo.grid(row=0, column=1, ipadx=0, ipady=0, padx=15, pady=0) 
clr = '#1f77b4'

btngoa = Button(frame_for_center4, command=analysis_choice, text='Визуализировать', bg='white', height=1, width=24, font=("Century Gothic", 10, "bold"))
btngoa.grid(row=0, column=2, ipadx=0, ipady=0, padx=15, pady=0)

title_file_name = Label(frame_for_center4, text='', bg='gray',fg='white', font=("Century Gothic", 10))
title_file_name.grid(row=1, column=0, ipadx=0, ipady=0, padx=0, pady=10, columnspan=3)
...
frame_0 = Frame(background="gray")
f0 = frame_0.place(in_=frame4, anchor="c", relx=.50, rely=.22)


frame_for_center0 = LabelFrame(frame_0, background="gray", text='Способ обработки', bg='gray', fg='white', font=("Century Gothic", 10), labelanchor="n")
frame_for_center0.grid(row=0, column=0, ipadx=0, ipady=0, padx=8, pady=0)

frame_for_hight = LabelFrame(frame_0, background="gray", text='Столбец для обработки', bg='gray', fg='white', font=("Century Gothic", 10), labelanchor="n")
frame_for_hight.grid(row=0, column=1, ipadx=0, ipady=0, padx=8, pady=0)  


frame_for_center = Frame(background="gray")
f2 = frame_for_center.place(in_=frame4, anchor="c", relx=.50, rely=.52)

frame_for_center2 = Frame(background="gray")
f3 = frame_for_center2.place(in_=frame4, anchor="c", relx=.50, rely=.69)
...
combobox = ttk.Combobox(frame_for_hight, textvariable=languages_var, height=10, width=17, values=languages, font=("Century Gothic", 12, "bold"))
combobox.grid(row=1, column=0, ipadx=0, ipady=0, padx=7, pady=5)
...
languages4 = ['География','Даты','Леммы','Коллокации']
languages_var4 = StringVar()
combobox4 = ttk.Combobox(frame_for_center0, textvariable=languages_var4, height=10, width=17, values=languages4, font=("Century Gothic", 12, "bold"))
languages_var4.set(languages4[0])
combobox4.grid(row=1, column=0, ipadx=5, ipady=0, padx=7, pady=7)


icon_plus_rep = PhotoImage(file="rep.png")
photoimage_rep = icon_plus_rep.subsample(2, 2)
btnm_ob = Button(frame_for_hight, image=photoimage_rep, command=rep, text='-', bg='white', height=23, width=23, font=("Century Gothic", 10, "bold"))
btnm_ob.grid(row=1, column=1, ipadx=0, ipady=0, padx=7, pady=5)
...
file_in2 = Button(frame_for_center, command=file_nm2, text='Выбрать файл', bg='white', height=1, width=24, font=("Century Gothic", 10, "bold"))
file_in2.grid(row=0, column=0, ipadx=0, ipady=0, padx=41, pady=0)

title_file_name2 = Label(frame_for_center, text='', bg='gray',fg='white', font=("Century Gothic", 10))
title_file_name2.grid(row=1, column=0, ipadx=0, ipady=0, padx=0, pady=10)


btngo_ob3 = Button(frame_for_center2, command=proc_data_choice, text='Обработать столбец', bg='white', height=1, width=25, font=("Century Gothic", 10, "bold"))
btngo_ob3.grid(row=0, column=0, ipadx=0, ipady=0, padx=15, pady=8)

btngo_ob4 = Button(frame_for_center2, command=delete_new_col, text='Удалить столбец', bg='white', height=1, width=25, font=("Century Gothic", 10, "bold"))
btngo_ob4.grid(row=0, column=1, ipadx=0, ipady=0, padx=15, pady=8)
